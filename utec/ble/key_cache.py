...
class BleKeyCache:
    ...
    def __init__(self, cache_file: Optional[str] = None):
        ...
        """
        self._cache: Dict[str, Tuple[bytes, float, str]] = {}  # mac -> (key, timestamp, method)
        self._lock = Lock()
        self._cache_file = cache_file or os.path.expanduser("~/.utec_key_cache.json")
        self._cache_ttl = 86400 * 7  # 7 days default TTL
    ...
    def get(self, mac_address: str) -> Optional[bytes]:
        ...
        with self._lock:
            ...
            if age > self._cache_ttl:
                logger.info(f"Cached key for {mac_address} expired (age: {age/3600:.1f}h)")
                del self._cache[mac_address.upper()]
                self._save_cache()
                return None
                
            logger.info(f"Using cached {method} key for {mac_address} (age: {age/3600:.1f}h)")
            return key
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        with self._lock:
            ...
            expired = sum(1 for _, (_, ts, _) in self._cache.items() 
